...
# Auth pour les reponses en streaming (SSE) : ouvre et referme sa propre session
# tout de suite. Sinon, via Depends(get_db), la connexion resterait mobilisee
# pendant toute la duree du flux (le generateur yield n'est finalise qu'a la fin
# de la reponse) et le pool finirait sature.
def require_admin_stream(token: str | None = Depends(auth_scheme)) -> Utilisateur:
    if not token:
        raise HTTPException(status_code=401, detail="Non authentifie")
    user_id = decode_access_token(token=token)
    if not user_id:
